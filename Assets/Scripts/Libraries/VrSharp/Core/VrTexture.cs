...
namespace VrSharp
{
    public abstract class VrTexture
    {
        ...
        /// <summary>
        /// Returns the decoded texture as a Unity Texture 2D
        /// </summary>
        /// <returns></returns>
        public Texture2D ToTexture2D()
        {
            ...
            Texture2D img = new Texture2D(textureWidth, textureHeight, TextureFormat.RGBA32, false);
            for (int i = 0; i < data.Length; i += 4) {
                // Change format from BGRA32 to RGBA32
                byte tmp = data[i];
                data[i] = data[i + 2];
                data[i + 2] = tmp;
            }
            img.LoadRawTextureData(data);
            img.Apply();
        ...
        public MemoryStream ToStream()
        {
            ...
            byte[] pngBytes = ToTexture2D().EncodeToPNG();
            destination.Write(pngBytes, 0, pngBytes.Length);
        ...
        /// </summary>
        /// <returns></returns>
        public Texture2D[] MipmapsToTexture2Ds()
        {
            if (!initalized)
            {
                throw new TextureNotInitalizedException("Cannot decode this texture as it is not initalized.");
            ...
            if (!HasMipmaps)
            {
                return new Texture2D[] { ToTexture2D() };
            }

            byte[][] data = DecodeMipmaps();

            Texture2D[] img = new Texture2D[data.Length];
            for (int i = 0, size = textureWidth; i < img.Length; i++, size >>= 1)
            {
                img[i] = new Texture2D(size, size, TextureFormat.RGBA32, false);
                ...
                img[i].LoadRawTextureData(data[i]);
                img[i].Apply();
            }
        ...
        public MemoryStream[] MipmapsToStream()
        {
            ...
            Texture2D[] img = MipmapsToTexture2Ds();

            MemoryStream[] destination = new MemoryStream[img.Length];
            for (int i = 0; i < img.Length; i++)
            {
                byte[] pngBytes = img[i].EncodeToPNG();
                destination[i].Write(pngBytes, 0, pngBytes.Length);
