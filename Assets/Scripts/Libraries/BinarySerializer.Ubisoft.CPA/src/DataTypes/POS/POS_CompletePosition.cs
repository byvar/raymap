...
namespace BinarySerializer.Ubisoft.CPA {
	// Transform matrix for R2 engine versions
	public class POS_CompletePosition : BinarySerializable, MAT_ITransform {
		public POS_TransformationType Type { get; set; }
		public MTH3D_Vector TranslationVector { get; set; }
		public MTH3D_Matrix RotationMatrix { get; set; }
		public MTH3D_Matrix TransformMatrix { get; set; } // Scale

		public override void SerializeImpl(SerializerObject s) {
			Type = s.Serialize<POS_TransformationType>(Type, name: nameof(Type));
			TranslationVector = s.SerializeObject<MTH3D_Vector>(TranslationVector, name: nameof(TranslationVector));
			RotationMatrix = s.SerializeObject<MTH3D_Matrix>(RotationMatrix, name: nameof(RotationMatrix));
			TransformMatrix = s.SerializeObject<MTH3D_Matrix>(TransformMatrix, name: nameof(TransformMatrix));
		}
