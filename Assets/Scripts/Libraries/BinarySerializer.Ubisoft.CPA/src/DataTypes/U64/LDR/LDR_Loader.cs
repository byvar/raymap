			...
			while (LoadQueue.First?.Value != null) {
				...
				if (off_struct == null) {
					if (!currentRef.IsGlobal) {
						...
						if (off_struct != null) {
							s.SystemLogger?.LogWarning($"Couldn't resolve Struct Reference {currentRef.Name} normally, successfully resolved globally");
						}
					}
					if (off_struct == null) {
						s.SystemLogger?.LogWarning($"Couldn't resolve Struct Reference {currentRef.Name}");
						continue;
					}
				}
