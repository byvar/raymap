    ...
    public class SNA : FileWithPointers {
        ...
        public void WriteSNA() {
            using (FileStream stream = new FileStream(path, FileMode.Open)) {
                Writer writer = new Writer(stream, Legacy_Settings.s.IsLittleEndian);
                ...
                foreach (SNAMemoryBlock block in blocks) {
                    ...
                    if (Legacy_Settings.s.engineVersion > Legacy_Settings.EngineVersion.Montreal) writer.Write(block.unk1);
                    ...
                    if (block.baseInMemory != -1) {
                        ...
                        if (Legacy_Settings.s.game == Legacy_Settings.Game.TT) writer.Write(block.unk1);
        ...
        void ReadSNA() {
            ...
            while (reader.BaseStream.Position < reader.BaseStream.Length) {
                ...
                if (Legacy_Settings.s.engineVersion > Legacy_Settings.EngineVersion.Montreal) block.unk1 = reader.ReadByte();
                ...
                if (block.baseInMemory != -1) {
                    ...
                    if (Legacy_Settings.s.game == Legacy_Settings.Game.TT) block.unk1 = reader.ReadByte();
        ...
        public void ReadGPT(string path, RelocationTable rtp) {
            ...
            byte[] gptData = null;
            using (Reader gptReader = new Reader(gptStream, Legacy_Settings.s.IsLittleEndian)) {
                int maskBytes = Legacy_Settings.s.encryptPointerFiles ? gptReader.InitMask() : 0;
        ...
        public void ReadDLG(string path, RelocationTable rtd) {
            ...
            byte[] dlgData = null;
            using (Reader dlgReader = new Reader(dlgStream, Legacy_Settings.s.IsLittleEndian)) {
                int maskBytes = Legacy_Settings.s.encryptPointerFiles ? dlgReader.InitMask() : 0;
        ...
        public void ReadSDA(string path) {
            ...
            byte[] sdaData = null;
            using (Reader sdaReader = new Reader(sdaStream, Legacy_Settings.s.IsLittleEndian)) {
        ...
        public void ReadPTX(string path, RelocationTable rtt) {
            ...
            byte[] ptxData = null;
            using (Reader ptxReader = new Reader(ptxStream, Legacy_Settings.s.IsLittleEndian)) {
                int maskBytes = Legacy_Settings.s.encryptPointerFiles ? ptxReader.InitMask() : 0;
                ...
                if (l.relocation_global.ContainsKey(ptrListRelocationKey)) {
                    ...
                    if (block_global.size > 0) {
                        foreach (RelocationPointerInfo info in ptrList.pointers) {
                            ...
                            if (!l.relocation_global.ContainsKey(ptrRelocationKey)) {
                                if (Legacy_Settings.s.engineVersion > Legacy_Settings.EngineVersion.Montreal || !(info.module == 0xFF && info.id == 0xFF)) {
