...
namespace OpenSpace.FileFormat.Texture {
	...
	public class TBF {
		...
		public TBF(string path, bool hasNames = false) {
			...
			using (Reader reader = new Reader(tbf, Legacy_Settings.s.IsLittleEndian)) {
		...
		private Color[] ReadPalette(Reader reader, int length, bool hasAlpha = true) {
			...
			Color[] pal = palette;
			if (length == 256 && (Legacy_Settings.s.game == Legacy_Settings.Game.R3 && Legacy_Settings.s.mode != Legacy_Settings.Mode.Rayman3PS2Demo_2002_05_17)) {
				// Tile
