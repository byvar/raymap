...
namespace OpenSpace.FileFormat.Texture {
    ...
    public class BTF {
		...
		public BTF(string btfPath, string bhfPath) {
			...
			using (Reader reader = new Reader(bhf, Legacy_Settings.s.IsLittleEndian)) {
				Count = reader.ReadUInt32();
				textures = new Texture2D[Count];
				if (Legacy_Settings.s.platform == Legacy_Settings.Platform.Xbox) {
			...
			using (Reader reader = new Reader(btf, Legacy_Settings.s.IsLittleEndian)) {
				if (Legacy_Settings.s.platform == Legacy_Settings.Platform.Xbox) {
					for (uint i = 0; i < Count; i++) {
