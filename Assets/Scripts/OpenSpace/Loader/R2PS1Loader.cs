...
namespace OpenSpace.Loader {
	public class R2PS1Loader : MapLoader {
		...
		public string[] LoadLevelList() {
			if (PS1GameInfo.Games.ContainsKey(Legacy_Settings.s.mode)) {
				return PS1GameInfo.Games[Legacy_Settings.s.mode].maps;
			}
		...
		protected override async UniTask Load() {
            try {
				...
				if (!PS1GameInfo.Games.ContainsKey(Legacy_Settings.s.mode)) throw new Exception("PS1 info wasn't defined for this mode");
				loadingState = "Initializing files";
				//RipRHRLoc();
				game = PS1GameInfo.Games[Legacy_Settings.s.mode];
		...
		private uint GetActorAddress(int i) {
			...
			switch (i) {
				...
				case 1:
					if (Legacy_Settings.s.game == Legacy_Settings.Game.JungleBook) {
						return (mainFile.memoryBlocks[CurrentLevel].overrideActor1Address ?? game.actor1Address)
		...
		public void ReadCollSetsR2(Reader reader) {
			if (Legacy_Settings.s.game != Legacy_Settings.Game.R2) return;
		...
		public async UniTask LoadLevel() {
			...
			if (Legacy_Settings.s.game == Legacy_Settings.Game.RRush || Legacy_Settings.s.game == Legacy_Settings.Game.JungleBook) {
		...
		public async UniTask LoadDataFromDAT(PS1GameInfo gameInfo, PS1GameInfo.File fileInfo, int index) {
			...
			if (FileSystem.FileExists(bigFilePath)) {
				using (Reader reader = new Reader(FileSystem.GetFileReadStream(bigFilePath), isLittleEndian: Legacy_Settings.s.IsLittleEndian)) {
					...
					if (fileInfo.type == PS1GameInfo.File.Type.Map) {
						...
						if (!b.inEngine) return;
						if (Legacy_Settings.s.game != Legacy_Settings.Game.RRush && !b.exeOnly) FileSystem.AddVirtualFile(levelDir + "vignette.tim", mainBlock[blockIndex++]);
			...
			if (FileSystem.FileExists(bigFilePath)) {
				using (Reader reader = new Reader(FileSystem.GetFileReadStream(bigFilePath))) {
					...
					for (int i = 0; i < memoryBlocks.Length; i++) {
						...
						int blockIndex = 0;
						if (fileInfo.type == PS1GameInfo.File.Type.Map) {
							if (Legacy_Settings.s.game != Legacy_Settings.Game.RRush && !b.exeOnly) Util.ByteArrayToFile(levelDir + "vignette.tim", mainBlock[blockIndex++]);
							if (!b.exeOnly && b.inEngine) {
								if (b.hasSoundEffects) {
		...
		public void ParseMainBlock(byte[] data, PS1GameInfo.File.MemoryBlock block, int index, string basename) {
			using (MemoryStream ms = new MemoryStream(data)) {
				using (Reader reader = new Reader(ms, Legacy_Settings.s.IsLittleEndian)) {
					reader.ReadUInt32();
					reader.ReadUInt32();
