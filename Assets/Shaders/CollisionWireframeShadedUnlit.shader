Shader "SuperSystems/CollisionWireframeShadedUnlit"
{
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(g2f i) : SV_Target
				{
					...
					float4 baseColor = _Color * tex2D(_MainTex, i.uv0);

					// Early out if we know we are not on a line segment.
					if (minDistanceToEdge > 0.9)
					{
						float mult = 1.0;
						if (i.normal.y < 0.7071067691) { // 1/sqrt(2)
							mult = 0.5;
						}
						else {
							mult = 1.0;
						...
						return fixed4(baseColor.rgb,0) * mult;
					}

					// Smooth our line out
					float t = exp2(_WireSmoothness * -1.0 * minDistanceToEdge * minDistanceToEdge);
					fixed4 invColor = fixed4(1.0 - _Color.r, 1.0 - _Color.g, 1.0 - _Color.b, 1.0);
					fixed4 finalColor = lerp(baseColor, invColor, t);
					finalColor.a = t;
